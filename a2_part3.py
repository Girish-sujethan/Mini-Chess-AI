...
def run_learning_algorithm(exploration_probabilities: list[float],
    ...
    for i in range(0, len(exploration_probabilities)):
        white_player = ExploringPlayer(game_tree, exploration_probabilities[i])
        black_player = a2_minichess.RandomPlayer()
        mini_chess_result = a2_minichess.run_game(white_player, black_player)
        game_tree.insert_move_sequence(mini_chess_result[1])
        results_so_far += mini_chess_result[0]
